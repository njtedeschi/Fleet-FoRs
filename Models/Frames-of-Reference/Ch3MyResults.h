...
public:
    ...
    TestingEvaluator(TopN<MyHypothesis> t) : top(t) {
    ...
    void evaluate(std::vector<TestingDatum> testing_data) {
        for (auto& testing_datum : testing_data) {
            ...
            for (auto& hypothesis : top.sorted()) {
                MyOutput output = hypothesis.call(testing_datum.context);
                const auto& judgments = output.first;
                for (auto& [word, sense_judgments] : judgments) {
                    std::string word_form = word.form;
                    const TruthValue& target_values = testing_datum.label.at(word_form);
                    bool predicted_value = false;
                    // predicted_value is true if any sense of word has judgment.value == JudgmentType::max_value
                    ...
                    for (auto& [test_sense, target_value] : target_values) {
                        RankWordSense key(rank, word_form, test_sense);
