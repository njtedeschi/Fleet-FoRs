...
MyHypothesis relative;
 ...
 // Set target concepts for words.
        ...
        // Relative concepts
        relative["above"] = InnerHypothesis(grammar.simple_parse("parallel(displacement(ground(x),figure(x)),up(x))"));
        relative["below"] = InnerHypothesis(grammar.simple_parse("parallel(displacement(figure(x),ground(x)),up(x))"));
        relative["front"] = InnerHypothesis(grammar.simple_parse("parallel(displacement(figure(x),ground(x)),orientation(speaker(x)))"));
        relative["behind"] = InnerHypothesis(grammar.simple_parse("parallel(displacement(ground(x),figure(x)),orientation(speaker(x)))"));
        relative["side"] = InnerHypothesis(grammar.simple_parse("orthogonal(displacement(ground(x),figure(x)),orientation(speaker(x)))"));

MyHypothesis::datum_t sample_datum(double p_direct, double p_intrinsic, double alpha_t) {
    // Sample scene
    Scene scene;
