        ...
        if [ -f "$zip_file" ]; then
            ...
            for testing_data_file in "$root_dir/testing_data"/*json; do
                if [ -f "$testing_data_file" ]; then
                    ...
                    ./test_3_${cpu_name} --language="$language" --testing_data_path="$testing_data_file" --model_directory="$tmp_dir/$subdirectory" --output_directory="$output_dir" --output_filename_stem="${subdirectory}_${test_name}"
