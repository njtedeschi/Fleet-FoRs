            ...
            for iteration in $iterations; do
                ...
                input_file="$json_dir/${training_size}_${iteration}.json"
                    if [ -f "$input_file" ]; then  # Check if it's a file
                        ...
                        ./train_3_${cpu_name} --chains="$chains" --threads="$threads" --steps="$steps" --top="$top" --language="$language" --input_path="$input_file" --output_path="$output_file"
